import React, { ChangeEvent, useEffect, useMemo, useState, CSSProperties, useCallback } from "react";
...
export default function UnsplashDock({ editorState, onBack, name }: SubMenuProps) {
  const [unslapshSearch, setUnsplashSearch] = useState<{items: any, DataisLoaded: boolean}>({items: [], DataisLoaded: false})
  const [searchKey, setSearchKey] = useState<string>("");
  
  function getImages(e: ChangeEvent<HTMLInputElement> | null){
  ...
  const fetchImages = useCallback(
    async () => {
      const res = await fetch(
          "https://api.unsplash.com/photos/?page=1&query="+searchKey+"&client_id=n3uKfn5DuxU6jNwLwMUb5ehAL-bDxCJBwY8gLj5F-Wo&sig=123")
  ...
  if (unslapshSearch?.DataisLoaded){
    console.log(unslapshSearch?.items)
  }else{
    getImages(null);
  }
    ...
    <div id={"subMenu" + name} className="subMenuWrapper">
      ...
      <div className="unsplashGallery">
        { unslapshSearch?.items.map((item : any,index: any) => (
          <img src={item.urls.regular} alt={item.alt_description} key={index} width="47%" onClick={()=>handleUnsplashImage(item.urls.raw, item.id.toString() )}/>
        ))}
      </div>
